...
The public documentation has one default English repository entry point at `README.md`, its Russian counterpart at `README.ru.md`, and one authoritative extended user guide at `docs/FULL-DOCUMENTATION.md`. Both README files link to each other above the hero image. The ignored legacy copies under `omp-compact/` remain removed rather than retained as alternative drafts. **Reason:** GitHub renders `README.md` by default, so making it English gives the repository an internationally accessible landing page while the explicit language switch preserves the edited Russian text; separating overview from reference still gives each fact one intended maintenance location. **Rejected:** leaving Russian as the implicit default, relying on browser translation, or restoring multiple legacy README drafts.