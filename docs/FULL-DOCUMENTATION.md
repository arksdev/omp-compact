...
- **Нечитаемый payload не сворачивается.** Если у показанного сообщения непустой
  ...
  лимита в 64 заметки), сворачивание отключается для всей сессии: такая карточка может
  спрятать нечитаемую запись за пределами первых трёх и нарисовать те же строки, что и
  доказанная, поэтому приписывать ей чужое доказательство нельзя. Карточки без заметок
  (`{}`, `{ "notes": [] }`) рисуют заголовок `Advisor 0 notes`, который не совпадает ни
  с одним кандидатом, и на остальные карточки не влияют. Отдельно от этого стоит
  бюджет: суммарный payload заметок сверх 1 МиБ переводит сессию в native-режим.
