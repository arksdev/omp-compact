...
`omp-compact` — presentation-плагин для OMP 18.0.0 и выше. Он показывает активность инструментов как компактный хронологический лог, а после завершения logical run оставляет только полезные для истории строки.
...
Поддерживаемый диапазон: **OMP 18.0.0 и выше**. Автоматический release gate и manual smoke закреплены на stock OMP 18.0.0; будущие версии считаются совместимыми, пока не меняют private TUI shape, от которого зависит presentation adapter.
...
Требуется OMP 18.0.0 или новее. Stock 18.0.0 остаётся pinned development/release host; newer hosts проходят runtime capability checks и fail-open при несовместимой private TUI shape.
...
| `host-adapter.ts` | Pinned 18.0.0 capability probes and transactional descriptor patches. |
...
- Проверьте, что runtime — OMP 18.0.0 или новее; для нового несовместимого shape приложите exact version и reproduction к GitHub issue.
