...
Поддерживаемый диапазон: **OMP 18.0.1 и выше**. Автоматический release gate и manual smoke закреплены на stock OMP 18.2.0; будущие версии считаются совместимыми, пока не меняют private TUI shape, от которого зависит presentation adapter.
...
Требуется OMP 18.0.1 или новее. Stock 18.2.0 остаётся pinned development/release host; newer hosts проходят runtime capability checks и fail-open при несовместимой private TUI shape.
...
| `host-adapter.ts` | Pinned 18.2.0 capability probes и transactional descriptor patches; re-exports the host-surface sheet so existing importers keep their path. |
