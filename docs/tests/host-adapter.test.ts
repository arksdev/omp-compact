...
describe("capability fingerprints", () => {
	...
	test("the host release pin targets OMP 18.0.3", () => {
		expect(HostAdapter1731.hostVersion).toBe("18.0.3");
	});
});
...
stockTest("stock 18.0.3 host capability canary", async () => {
	...
	expect(stockHostVersion()).toBe("18.0.3");
});

stockTest(
	"stock 18.0.3 transcript forwards activity visibility to new children",
	async () => {
		const host = await loadStockHost();
		const transcript = new host.TranscriptContainer();
