...
stockTest("stock 18.3.2 host capability canary", async () => {
	...
	expect(stockHostVersion()).toBe("18.3.2");
});

stockTest(
	"stock 18.3.2 transcript forwards activity visibility to new children",
	async () => {
		const host = await loadStockHost();
		const transcript = new host.TranscriptContainer();
