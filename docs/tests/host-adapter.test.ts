...
describe("capability fingerprints", () => {
	...
	test("the host release pin targets OMP 18.3.0", () => {
		expect(StockHostAdapter.hostVersion).toBe("18.3.0");
	});
});
...
stockTest("stock 18.3.0 host capability canary", async () => {
	...
	expect(stockHostVersion()).toBe("18.3.0");
});

stockTest(
	"stock 18.3.0 transcript forwards activity visibility to new children",
	async () => {
		const host = await loadStockHost();
		const transcript = new host.TranscriptContainer();
