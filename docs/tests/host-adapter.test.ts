...
describe("capability fingerprints", () => {
	...
	test("the host release pin targets OMP 18.2.6", () => {
		expect(StockHostAdapter.hostVersion).toBe("18.2.6");
	});
});
...
stockTest("stock 18.2.6 host capability canary", async () => {
	...
	expect(stockHostVersion()).toBe("18.2.6");
});

stockTest(
	"stock 18.2.6 transcript forwards activity visibility to new children",
	async () => {
		const host = await loadStockHost();
		const transcript = new host.TranscriptContainer();
