...
describe("capability fingerprints", () => {
	...
	test("the host release pin targets OMP 18.1.10", () => {
		expect(StockHostAdapter.hostVersion).toBe("18.1.10");
	});
});
...
stockTest("stock 18.1.10 host capability canary", async () => {
	...
	expect(stockHostVersion()).toBe("18.1.10");
});

stockTest(
	"stock 18.1.10 transcript forwards activity visibility to new children",
	async () => {
		const host = await loadStockHost();
		const transcript = new host.TranscriptContainer();
