 ...
 *
 * `StockHostAdapter.hostVersion` (`"18.2.11"`) is the **verified contract**
 ...
 * The gate pin is 18.2.11 (root `node_modules`), verified from an isolated
 * `runtime/omp-18.2.11/` install before the root tree moved.
 ...
 * `runtime/omp-18.2.9/PIN-MOVE-18.2.9.md` and
 ...
 * the 18.2.11 pin and resolved via live capability probes on the instance;
...
export function isSkillMessageComponent(
	...
	if (message.customType === "skill-prompt") return true;
	// OMP >= 18.2.5 moved #message to a private field and introduced
	// markTranscriptBlockPending / markTranscriptBlockFinalized.
	return (
		typeof candidate.markTranscriptBlockPending === "function" &&
		typeof candidate.markTranscriptBlockFinalized === "function"
	);
}
...
export function isLateDiagnosticsMessageComponent(
	...
	if (typeof candidate.removeEntry === "function") return false;
	if (typeof candidate.renameEntry === "function") return false;
	if (Array.isArray(candidate.files)) return true;
	// OMP >= 18.2.5 moved files to #files private field and wraps its tree in a Disclosure.
	// ToolActivityContainer collides on the 3 methods above but wraps tool activities, not Disclosure.
	if (candidate.constructor?.name === "LateDiagnosticsMessageComponent")
		return true;
	if (Array.isArray(candidate.children) && candidate.children.length === 1) {
		const child = candidate.children[0] as Record<string, unknown> | undefined;
		if (child?.constructor?.name === "Disclosure") return true;
	}
	return false;
}
