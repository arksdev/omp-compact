...
- Хост-пин переехал с 18.2.0 на 18.2.5. Масштабная архитектурная модулизация: TUI-компоненты чата, хром и движок тем перенесены из `@oh-my-pi/pi-coding-agent` в `@oh-my-pi/pi-tui`. Все 8 патчируемых методов `TranscriptContainer` и 8 методов `ToolExecutionComponent` полностью сохранены по сигнатурам и арностям. Группа чтений, активность инструментов и `resetDisplay()` (`\x1b[3J`) по-прежнему на месте. Типы темы отвязаны от внутренних путей хоста через структурный интерфейс `theme-types.ts`. Компоненты `SkillMessageComponent` и `LateDiagnosticsMessageComponent` перешли на нативные приватные поля ECMAScript (`#message`, `#files`), для них реализовано безопасное распознавание и скрейпинг дочерних узлов.
