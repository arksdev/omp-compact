...
- Хост-пин переехал с 18.1.10 на 18.1.14 (через 18.1.11-18.1.13). Небольшой шаг: 54 изменённых пути, два новых файла, ни одного удалённого. Вся поверхность, на которой работает плагин, побайтово та же — контейнер транскрипта, карточка инструмента, группа чтений, контроллер событий, composer, — и оба модуля, которые плагин импортирует напрямую, не изменились. Релиз резервирует доставку вставки из буфера, взводит inline `/loop` только после подтверждения отправки запроса и откладывает фоновую компакцию, пока не завершится фоновая задача. Правок в плагине не потребовалось.
- Хост-пин переехал с 18.1.14 на 18.1.15. Ещё меньший шаг: 46 изменённых путей, ни одного добавленного или удалённого файла, и всё, на чём работает плагин, побайтово то же. Впервые с момента исправления прокрутки изменился `pi-tui` — релиз добавляет перерисовку «на месте» при изменении размера окна для Warp, — поэтому отдельно подтверждено, что сброс экрана по-прежнему стирает сохранённую прокрутку: именно на этом держится возможность свернуть строки в конце хода. Остальное в релизе — бюджет заметок советчика и заморозка простаивающих вкладок браузера. Правок в плагине не потребовалось.
...
- Хост-пин переехал с 18.1.17 на 18.2.0 (минорный релиз OMP). Изменено 237 путей агента, появились спекулятивное исполнение, совместная работа, кэширование промптов и клик-фокус мыши по субагентам (`tui.mouse`). Вся поверхность, с которой взаимодействует плагин, полностью сохранена: все 8 методов контейнера транскрипта и 8 методов карточек инструментов совпадают по сигнатурам и поведению, группа чтений, роутер, keybindings и composer побайтово те же; все 13 проб методов прошли. `resetDisplay()` в `pi-tui` по-прежнему стирает историю через единственный `\x1b[3J`. Правок в плагине не потребовалось.
