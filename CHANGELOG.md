...
- Восстановлена загрузка плагина в скомпилированном OMP 18.2.5 после переноса UI-модулей в `pi-tui`. Импорты keybindings и типов темы используют стабильный корневой экспорт; две недоступные внутренние вспомогательные функции сохранены локально с регрессионными тестами. Development host обновлён до 18.2.5. Карточки навыков и поздней диагностики с закрытыми полями остаются штатными.

### Fixed

- Restore plugin loading in compiled OMP 18.2.5 after the UI package split. Use stable root exports for keybindings and theme types, and preserve the two unavailable internal helpers locally with behavioral regression coverage. Update the development host to 18.2.5. Skill and late-diagnostics cards with private host fields remain native.
