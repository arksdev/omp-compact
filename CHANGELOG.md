...
- После автоматической встряски контекста (auto-shake) хост перерисовывает весь протокол заново, и в длинной сессии карточки инструментов оставались в родном виде до переоткрытия сессии — теперь перерисовка после встряски связывает видимый хвост и строки остаются компактными.
- Тот же компактный вид сохраняется и при остальных перерисовках протокола, у которых нет события: ручная встряска `/shake`, отмена только что отправленного промпта, возврат промпта, отклонённого до отправки, перерисовка от расширения. Восстановленная карточка теперь опознаётся по собственному идентификатору вызова, который хост повторяет вместе с результатом, а не по совпадению числа карточек с числом вызовов в истории — а именно это совпадение и ломала свёрнутая история.
- Чтение по внутренней ссылке (`read skill://…`, `omp://…`) рядом с другим инструментом в одном ответе модели больше не ломает компактный вид: раньше такая пара — например `read skill://keep-the-why-distilled` и `bash printenv HOME` — показывалась в рамках родного вида, причём до конца запуска, а после переоткрытия той же сессии выглядела компактно. Компактные строки появляются сразу, вживую.
- Карточка инструмента, который плагин не показывает компактно — любой вызов MCP-сервера, устройства `xd://`, инструмент без правила показа, — больше не уводит в родной вид соседние строки. Состояния теперь резервируются в том же порядке, в каком хост создаёт карточки, поэтому позиционное сопоставление не перекрещивает MCP-карточку с состоянием `bash`; раньше такое сообщение оставляло обе карточки в рамке до конца прогона.
- Коммит, текст сообщения которого собран подстановкой — `git commit -q -m "$(printf 'заголовок\n\nтело')"` — теперь виден в журнале. Разбор git-цепочек отвергал такую команду целиком, поэтому ход, в котором коммит действительно был сделан, не оставлял после ответа ни строки, ни итоговой сводки: журнал утверждал, что ничего не происходило. Теперь подстановка принимается, если внутри неё только `printf` или `echo` с одним литеральным аргументом — то есть она заведомо ничего не выполняет; всё остальное (`$(git …)`, `$(cat …)`, конвертации `%s`, вложенные подстановки) по-прежнему отвергается целиком.
- Список в фигурных скобках внутри пути — `git add src/hud/{scene.rs,panel.rs}` — тоже больше не отвергает всю цепочку. Скобки показываются ровно так, как были набраны, и не раскрываются; групповая команда `{ … ; }` по-прежнему отвергается, потому что меняет границы команд.
- Тихий коммит (`git commit -q`) не печатает сводку, поэтому его хэш раньше было нечем доказать и он не попадал в итоговую строку `• git commit: …`. Если сразу за коммитом в той же цепочке идёт `git log`, хэш берётся из первой строки вывода — но только когда её тема буква в букву совпадает с темой коммита, а до коммита в цепочке не было ни одного `git log`.
...
- Плагин проверен на OMP 18.1.3 и теперь собирается против него; минимально поддерживаемая версия OMP не изменилась — 18.0.1. Дописывать ничего не пришлось: журнал, карточка инструмента, группа чтений и контроллер событий в 18.1.2 и 18.1.3 остались побайтово прежними, а набор мест, где хост перерисовывает протокол целиком, не изменился.
- Два изменения хоста, которые видны в компактном виде. Двойной Escape на сообщении пользователя больше не отводит ветку в новую сессию, а переходит по дереву текущей — протокол после этого перерисовывается тем же путём, который плагин уже отслеживает. И если модель напечатала правку файла обычным текстом вместо вызова инструмента, хост сам превращает её в настоящий вызов `edit` (настройка `edit.recoverInlineEdits`, включена по умолчанию) — такая правка показывается обычной компактной строкой.
- Установка плагина: OMP 18.1.3 перестал загружать каталоги из `~/.omp/plugins/node_modules`, которые не заявлены в зависимостях, — то есть остатки от прежних установок. Каталог, подключённый ссылкой (`omp plugin link`), по-прежнему загружается, поэтому рабочая копия из git продолжает работать как раньше.
- Плагин проверен на OMP 18.1.4 и теперь собирается против него; минимально поддерживаемая версия OMP по-прежнему 18.0.1. В самом хосте править было нечего: исходники `pi-coding-agent` (1729 файлов) и `pi-tui` (50 файлов) в 18.1.4 побайтово совпадают с 18.1.3. Релиз обновил каталог моделей (`gemini-3.8-flash` у пяти провайдеров) и правила совместимости провайдеров — плагин их не читает.
