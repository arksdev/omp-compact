...
- Обновлена поддерживаемая версия OMP до 18.2.11. Публичный диапазон `engines.omp >=18.0.1` остаётся без изменений.

---

### Changed

- Updated supported OMP version to 18.2.11. Public range `engines.omp >=18.0.1` remains unchanged.
...
- Поддержка тем вынесена в отдельный модуль для улучшения работы плагина.

### Added

- Theme support moved to a dedicated module for improved plugin operation.

### Исправлено

- Коммиты с многострочными сообщениями больше не пропадают из журнала. Если агент или пользователь создавал коммит с телом через перенос строки (`git commit -am "заголовок\n\nтело"`), токенизатор команд считал символ переноса строки внутри кавычек недопустимым и молча отбрасывал команду целиком: карточка не рисовалась во время работы и не попадала в сводку `• git commit: ...` после хода. Теперь переносы строк внутри кавычек, экранированные переносы `\` и команды, разделённые переносами строк, разбираются штатно.

### Fixed

- Multiline git commit messages no longer disappear from the log. When an agent or user created a commit with a body separated by newlines (`git commit -am "subject\n\nbody"`), the shell command tokenizer treated newlines inside quotes as invalid and silently dropped the command: no card was rendered during the run and no `• git commit: ...` summary was emitted at the end of the turn. Newlines inside quotes, line continuations `\`, and newline-separated commands are now parsed as expected.

### Изменено

- Хост-пин переехал на OMP 18.2.5. Масштабная архитектурная модулизация: TUI-компоненты чата, хром и движок тем перенесены из `@oh-my-pi/pi-coding-agent` в `@oh-my-pi/pi-tui`. Все 8 патчируемых методов `TranscriptContainer` и 8 методов `ToolExecutionComponent` полностью сохранены по сигнатурам и арностям. Группа чтений, активность инструментов и `resetDisplay()` (`\x1b[3J`) по-прежнему на месте. Типы темы отвязаны от внутренних путей хоста через структурный интерфейс `theme-types.ts`. Компоненты `SkillMessageComponent` и `LateDiagnosticsMessageComponent` перешли на нативные приватные поля ECMAScript (`#message`, `#files`), для них реализовано безопасное распознавание и скрейпинг дочерних узлов.
...
- Исправлено отображение карточек инструментов при чтении рядом с другим инструментом.
...
- Исправлена совместимость с протоколом вывода инструментов последней версии.
