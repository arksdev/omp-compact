...
Управление параллельными рабочими сессиями тоже показывается коротко: вместо больших рамок с «телевизором» на каждую сессию — одна‑две строки с состоянием, именем, числом сделанных ходов, длительностью текущего хода, моделью и текущим занятием. Пока сессия работает, её строка живая. Снятые сессии и погашенные задачи недолго остаются на виду и исчезают сами; когда показывать нечего, строки нет вовсе.
...
Опция включает короткие строки для пяти инструментов управления параллельными рабочими сессиями. По умолчанию она включена.

Если её выключить, эти инструменты рисуются штатными карточками OMP, как если бы плагин их не знал: строки не скрываются и не заменяются обычным коротким описанием. Значение фиксируется на весь текущий logical run, поэтому сохранение настроек посреди работы не меняет картинку на полпути.
