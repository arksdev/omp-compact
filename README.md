...
Часть инструментов OMP вызывается через тот же транспорт, что и запись файла. Такие вызовы больше не выглядят записью: в строке видно само устройство и его операцию — например `github: pr_create` вместо `write: xd://github`, — и в учёт изменений файлов они не попадают.

Работа в режиме `/vibe` стала компактнее: вместо вывода уже убитых или простаивающих сабагентов выводятся только активные сабагенты. На каждую сессию приходится одна‑две короткие строки — состояние, имя, сколько ходов сделано, сколько времени идёт текущий, какая модель и чем сессия занята прямо сейчас. Убитые сессии и завершённые ходы задерживаются на несколько секунд и исчезают сами; когда показывать нечего, строки нет вовсе.
...
### Параллельные рабочие сессии — `Worker sessions`

Опция включает короткий вид параллельных рабочих сессий в режиме `/vibe`. По умолчанию она включена.

Если её выключить — эти сессии снова показываются штатными карточками OMP, как если бы плагин их не знал: строки не скрываются и не заменяются обычным коротким описанием. Значение фиксируется на весь текущий logical run, поэтому сохранение настроек посреди работы не меняет картинку на полпути.
...
Поддерживаемый диапазон — **OMP 17.4.2 и выше**. Release gate закреплён на stock OMP 17.4.2; будущие версии считаются совместимыми, пока не изменят private TUI shape. При таком изменении capability checks fail-open возвращают native rendering — укажите версию OMP и reproduction в GitHub issue.
