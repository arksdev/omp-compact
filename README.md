...
- В `clear` промежуточные Git rows скрыты вместе с остальными обычными строками, но строка с hashes созданных commits после ответа остаётся: созданные commits — единственное, что спокойный режим не прячет. Если опцию выключить, не остаётся и она.
